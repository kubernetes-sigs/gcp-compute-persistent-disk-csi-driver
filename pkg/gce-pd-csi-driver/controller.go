...
func (gceCS *GCEControllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if content != nil {
		...
		if content.GetVolume() != nil {
			...
			// Verify the disk location type, disk type, and encryption key(if used) matches the source disk.
			if err := gce.ValidateDiskParameters(diskFromSourceVolume, params); err != nil {
				return nil, status.Errorf(codes.InvalidArgument, `CreateVolume source volume parameters do not match CreateVolumeRequest Parameters %v: %v`, params, err)
			}

			// For zonal->zonal disk clones, verify the zone is the same as that of the source disk.
			if params.ReplicationType == replicationTypeNone && sourceVolKey.Zone != volKey.Zone {
