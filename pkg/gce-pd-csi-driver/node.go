...
func (ns *GCENodeServer) NodeUnstageVolume(ctx context.Context, req *csi.NodeUnstageVolumeRequest) (*csi.NodeUnstageVolumeResponse, error) {
	...
	if err != nil {
		...
		if devFsPath, err := filepath.EvalSymlinks(devicePath); err != nil {
			klog.Errorf("filepath.EvalSymlinks(%q) failed when trying to disable device: %v (ignored, unstaging continues)", devicePath, err)
		} else if err := ns.DeviceUtils.DisableDevice(devFsPath); err != nil {
			klog.Errorf("Failed to disabled device %s (aka %s) for volume %s. Device may not be detached cleanly (error is ignored and unstaging is continuing): %v", devicePath, devFsPath, volumeID, err)
		}
	}
