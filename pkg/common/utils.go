...
func CodeForError(sourceError error) codes.Code {
	...
	var apiErrv2 *apiv2error.APIError
	if !errors.As(sourceError, &apiErrv2) {
		klog.Infof("CodeForError: sourceErr cannot be converted to apiv2error: %+v", apiErrv2)
	}
	klog.Infof("CodeForError: sourceErr was converted to apiv2error: %+v", apiErrv2)
